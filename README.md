...
I used LangGraph to create a graph has two phases — **ingest** (fetch → parse → merge) and **query** (plan → review → validate → execute). All paths route to a single `respond` node with `status` of `ok` or `error`.
...
- **Plan node** — `plan` runs the full LLM tool-calling loop internally (no separate tool-execution graph node).
